...
use Jobs\ZohoSDK\com\zoho\crm\library\crud\ZCRMJunctionRecord;
use Jobs\ZohoSDK\com\zoho\crm\library\crud\ZCRMNote;
use Jobs\ZohoSDK\com\zoho\crm\library\crud\ZCRMRecord;
use Jobs\ZohoSDK\com\zoho\crm\library\exception\ZCRMException;
use Jobs\ZohoSDK\Test\zoho\crm\library\common\Helper;
use Jobs\ZohoSDK\Test\zoho\crm\library\common\TestUtil;
use Jobs\ZohoSDK\Test\zoho\crm\library\Main;
...
class EntityAPIHandlerTest
{
    ...
    public function testCreateRecord()
    {
        ...
        $moduleList=TestUtil::moveModulePositions(true, ['Products'], MetaDataAPIHandlerTest::$moduleList);
    ...
    public function testDeleteRecord()
    {
        ...
        try {
            $moduleList=TestUtil::moveModulePositions(true, ['Attachments','Notes'], MetaDataAPIHandlerTest::$moduleList);
            $moduleList=TestUtil::moveModulePositions(false, ['Products'], $moduleList);
            foreach ($moduleList as $apiName=>$moduleName) {
                $startTime=microtime(true)*1000;
                $endTime=0;
                try {
            ...
            foreach ($layoutIds as $layoutId) {
                ...
                foreach ($moduleFields as $fieldAPIName=>$fieldDetails) {
                    ...
                    if ($moduleName=='PriceBooks' && $fieldLabel=='Pricing Details') {
                        $pricingDetails=['from_range'=>1,'to_range'=>100,'discount'=>5];
                        $zcrmrecord->setFieldValue($fieldAPIName, [$pricingDetails]);
                    } elseif ($fieldLabel=='Product Details' && ($moduleName=='Quotes' || $moduleName=='SalesOrders' || $moduleName=='PurchaseOrders'|| $moduleName='Invoices')) {
                        $productObj=['id'=>self::$productId];
                        $productDetailsObj=['product'=>$productObj,'quantity'=>150];
                        $zcrmrecord->setFieldValue($fieldAPIName, [$productDetailsObj]);
                        ...
                        $participantObj=['type'=>'user','participant'=>OrganizationAPIHandlerTest::$userIdList[0]];
                        $zcrmrecord->setFieldValue($fieldAPIName, [$participantObj]);
                    }
                }
                if ($moduleName=='Notes') {
    ...
    public static function alterModulePositions($isProductFirst)
    {
        ...
        foreach (MetaDataAPIHandlerTest::$moduleList as $apiName=>$moduleName) {
            if ($isProductFirst) {
                ...
                } elseif (($moduleName=='Quotes' || $moduleName=='SalesOrders' || $moduleName=='PurchaseOrders' || $moduleName=='Invoices') && !$productModuleFound) {
                    $swapNeeded=true;
                }
            } else {
                if ($moduleName=='Products') {
                ...
                } elseif ($moduleName=='Quotes' || $moduleName=='SalesOrders' || $moduleName=='PurchaseOrders' || $moduleName=='Invoices') {
        ...
        if ($swapNeeded) {
            ...
            if ($isProductFirst) {
                $moduleList=[$productAPIName=>'Products']+$moduleList;
            } else {
                $moduleList=$moduleList+[$productAPIName=>'Products'];
            }
        }
